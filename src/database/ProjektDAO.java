...
public class ProjektDAO
{
   ...
   {
      final TypedQuery<Projekt> typedQuery = em.createQuery(
            "Select p from Projekt p where status = :status", Projekt.class);
      typedQuery.setParameter("status", status);
      return typedQuery.getResultList();
   }

   public void evaluateProject(Projekt projekt)
   {
      try
      {
      ...
      {
         System.out.println(
               "failed to update Projekt " + projekt.getProjekt_id());
         em.getTransaction().rollback();
      }
   }
